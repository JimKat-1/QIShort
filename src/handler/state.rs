...
//         // let mut_ref = &mut **self.current_branch.as_mut().ok_or(anyhow!(HandlerError::NoBranch))?;
//         // let action = mut_ref.get_mut(&BranchKey::Key(key)).ok_or(anyhow!(HandlerError::KeyNotFound))?;
//         let mut_ref = match (&mut self.current_branch) {
//             Some(a) => { &mut **a },
